...
prompt_kubectl() {
  ...
  if which kubectl &> /dev/null; then
    prompt_segment $BULLETTRAIN_KUBE_BG $BULLETTRAIN_KUBE_FG $BULLETTRAIN_KUBE_PREFIX" $(kubectl config view | grep current-context | cut -d' ' -f2)"
  fi
}
